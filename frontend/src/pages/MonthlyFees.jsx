...
const MonthlyFees = () => {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const data = await feeService.getStudents('monthly');
      setStudents(data);
    } catch (err) {
  ...
  const handleExport = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, detailWS, 'Monthly_Fees_Detail');
      XLSX.utils.book_append_sheet(wb, summaryWS, 'Monthly_Fees_Summary');
      XLSX.writeFile(wb, `monthly_fees_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleBackup = () => {
    try {
      ...
      link.href = url;
      link.download = `monthly_fees_backup_${new Date().toISOString().split('T')[0]}.json`;
      link.click();
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (Array.isArray(backupData)) {
            ...
            await feeService.resetData('monthly');
            ...
            await Promise.all(
              backupData.map(record => 
                feeService.addStudent({
                  ...
                  studentName: record.name || record.studentName, // Handle both name formats
  ...
  const handleReset = async () => {
    if (window.confirm('Are you sure you want to reset all monthly fee data? This cannot be undone!')) {
      try {
        await feeService.resetData('monthly');
        toast.success('All monthly fee data has been reset');
        fetchStudents();
      } catch (err) {
